# 預算系統開發任務分配

> 📄 **技術規格文件**: `/docs/specs/budget-system-spec.md`

---

## 🗄️ Task 1: Database Schema

**Assignee**: ****\_****  
**Priority**: High  
**Estimated**: 2-3 days  
**Phase**: 1 (與 Task 2 同時進行)

### 目標

建立預算系統所需的資料表結構。

### 工作項目

1. **建立 `Budget` 表** - 參考規格 Section 2.1
2. **建立 `BudgetCategory` 表** - 參考規格 Section 2.2
3. **建立 `TransactionBudget` 表** - 參考規格 Section 2.3
4. **建立 `BudgetPeriodSnapshot` 表** - 參考規格 Section 2.4
5. **建立 Migration 檔案** (Sequelize)
6. **建立 Seed 檔案** (測試資料)

### 驗收標準

- [ ] Migration 可正常執行 (`pnpm db:migrate`)
- [ ] 所有 FK 關聯正確
- [ ] Index 建立在常用查詢欄位

---

## 🎨 Task 2: Frontend UI (Mock Data)

**Assignee**: ****\_****  
**Priority**: High  
**Estimated**: 4-5 days  
**Phase**: 1 (與 Task 1 同時進行)

### 目標

使用假資料建立預算系統前端介面，符合 API 規格結構。

### 依賴

- 無（使用 Mock Data）

### 工作項目

#### 2.1 Mock Data 建立

1. **建立 Mock Service** (`/services/mock/budgetMock.ts`)
   - 模擬 API Response 結構
   - 包含：Budget 列表、詳情、使用率、歷史 Snapshots
   - 模擬延遲 (simulate loading state)

#### 2.2 UI 元件

2. **預算列表頁面** - 參考規格 Section 5.2
3. **預算新增/編輯表單** - 參考規格 Section 5.1
4. **預算修改確認 Modal** - 參考規格 Section 5.3
5. **交易表單「歸入預算」欄位** - 參考規格 Section 5.5
6. **重算中狀態 UI** - Loading skeleton + 禁用按鈕
7. **回溯補帳確認 Modal** - 參考規格 Section 3.6
8. **首頁預算 Widget** - 參考規格 Section 5.2

### 驗收標準

- [ ] 所有頁面響應式
- [ ] Light/Dark mode 支援
- [ ] Mock Data 結構與 API 規格一致
- [ ] 可切換 Mock/Real API (環境變數)

---

## ⚙️ Task 3: Backend API

**Assignee**: ****\_****  
**Priority**: High  
**Estimated**: 5-7 days  
**Phase**: 2 (等待 Phase 1 完成)

### 目標

實作預算系統的後端 API 與業務邏輯。

### 依賴

- ✅ Task 1 (Database) 完成

### 工作項目

#### 3.1 Core CRUD

1. **Budget API** - 參考規格 Section 4.1
2. **BudgetCategory API** - 參考規格 Section 4.2
3. **Statistics API** - 參考規格 Section 4.3

#### 3.2 Business Logic

4. **週期計算** - 參考規格 Section 3.1
5. **使用率計算** - 參考規格 Section 3.3
6. **修改生效時間** - 參考規格 Section 3.4

#### 3.3 Advanced

7. **Alert 觸發** - 參考規格 Section 3.5
8. **回溯補帳處理** - 參考規格 Section 3.6
9. **Snapshot 惰性建立** - 參考規格 Section 3.7
10. **修改 Transaction Service** - 儲存 TransactionBudget 關聯

### 驗收標準

- [ ] 所有 API 可正常運作
- [ ] Unit Test 覆蓋核心邏輯 (follow `backend-testing-standard`)
- [ ] 回溯補帳可觸發重算

---

## 🔗 Task 4: Frontend API Integration

**Assignee**: ****\_****  
**Priority**: High  
**Estimated**: 2-3 days  
**Phase**: 3 (等待 Phase 2 完成)

### 目標

將前端從 Mock Data 切換為真實 API。

### 依賴

- ✅ Task 2 (Frontend UI) 完成
- ✅ Task 3 (Backend API) 完成

### 工作項目

1. **建立 API Service** (`/services/api/budgetApi.ts`)
   - 替換 Mock Service
   - 統一錯誤處理

2. **串接所有 API Endpoints**
   - Budget CRUD
   - BudgetCategory CRUD
   - Statistics API

3. **實作 WebSocket/SSE**
   - 接收重算完成通知
   - 瀏覽器通知

4. **E2E 測試**
   - 完整流程測試

### 驗收標準

- [ ] 所有功能與真實 API 正常運作
- [ ] 錯誤處理完善
- [ ] E2E Test 通過

---

## 📅 開發時程

```
┌─────────────────────────────────────────────────────────────┐
│ Phase 1 (Week 1)                                            │
│ ┌─────────────────────┐ ┌─────────────────────────────────┐ │
│ │ Task 1: DB Schema   │ │ Task 2: Frontend UI (Mock)      │ │
│ │ 2-3 days            │ │ 4-5 days                        │ │
│ └─────────────────────┘ └─────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────┤
│ Phase 2 (Week 2)                                            │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ Task 3: Backend API                                     │ │
│ │ 5-7 days                                                │ │
│ └─────────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────┤
│ Phase 3 (Week 3)                                            │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ Task 4: Frontend API Integration + E2E Testing          │ │
│ │ 2-3 days                                                │ │
│ └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
