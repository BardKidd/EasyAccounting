...
// --- Logic Functions (Exported for Testing) ---
...
export const checkWeeklySummaryNotice = async () => {
    ...
    for (const sub of subscriptions) {
      ...
      if (incomeData.length > 0) {
        incomeSummaryData = {
          ...
          doughnutlabel: incomeData.reduce((acc, item) => acc + item.amount, 0),
        };
      } else {
        incomeSummaryData = {
          labels: [],
...
export const checkMonthlyAnalysisNotice = async () => {
