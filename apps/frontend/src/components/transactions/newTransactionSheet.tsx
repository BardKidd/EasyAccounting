...
import { BackdatingConfirmationModal } from './BackdatingConfirmationModal';
...
function NewTransactionSheet({
  ...
  const [pendingTransactionData, setPendingTransactionData] = useState<TransactionFormSchema | null>(null);
  ...
  const handleConfirmBackdate = async () => {
    if (pendingTransactionData) {
        await submitTransaction(pendingTransactionData);
        setShowBackdateConfirm(false);
        setPendingTransactionData(null);
    }
  };

  const submitTransaction = async (data: TransactionFormSchema) => {
    // 整理成 API 需要的格式
    ...
    try {
      ...
      if (result?.isSuccess) {
        ...
        form.reset();
        setShowExtra(false);
        setSelectedBudgetIds([]); // Clear budget selection
      }
    } catch (err) {
  ...
  return (
    <>
                            ...
                            >
                              <Checkbox 
                                  ...
                                  onCheckedChange={(checked) => {
                                      // Managed by parent div click for better UX, but handle here too just in case
                                      setSelectedBudgetIds((prev) =>
                                          checked
                                          ? [...prev, b.id]
                                          : prev.filter((p) => p !== b.id)
      ...
      <BackdatingConfirmationModal 
        ...
        date={pendingTransactionData?.date || new Date()}
      />
    </>
  );
}
